...
const Utils = {
  ...
  getFormatFloat:(val)=>{
    ...
    let idx = str.indexOf('.') ;
    if(idx != -1){
      if(str.slice(idx+1).length < 2){
         for(let i=0; i<(2-str.slice(idx+1).length); i++){
           str += '0'
         }       
      }
    }else{
  ...
  GetFontColor: (hexString) => {
    ...
    let luminance =
      (0.299 * backColor.r + 0.587 * backColor.g + 0.114 * backColor.b) / 255;
