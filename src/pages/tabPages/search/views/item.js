...
export default Item = ({ item, onTapItem }) => {
  ...
  let OfferCount = item?.offers ?? 0;
  OfferCount = isNaN(OfferCount) ? 0 : OfferCount;

  let DisplayValue = item?.title ?? "Unknown";
  let asin = item?.asin ?? "--";
  ...
  if (setting.use_ideal_buy == 1) {
    isShowTag =
      setting.amz_is_not_merchnat == 1
        ? isShowTag && is_amazon == 0
        : isShowTag;
