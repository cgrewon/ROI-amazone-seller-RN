...
export const FillButton = ({title, style, onPress, icon}) => {
  let height = style?.height ?? 40;

  let width = style?.width ?? '100%';
  let fontSize = style?.fontSize ?? 15;
  let color = style?.color ?? Constants.white;
  let backColor = style?.backgroundColor ?? Constants.green;
...
export const ToggleButton = ({title, selected, style, onPress}) => {
  ...
  let width = style?.width ?? '100%';
  let fontSize = style?.fontSize ?? 15;
  let color = style?.color ? style.color : Constants.primaryColor;
